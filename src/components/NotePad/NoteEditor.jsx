...
const CombinedEditor = ({ selectedNote,  newChatTriggered, refreshNotes   }) => {
  ...
  useEffect(() => {
    if (selectedNote && editor) {
      ...
      // Use editor's setContent method to set the content safely
      editor.commands.setContent(selectedNote.content || "<p></p>");

      // Save note ID to localStorage
  ...
  useEffect(() => {
    if (selectedNote) {
      ...
      // Assuming you have a reference to the contenteditable div
      const contentEditableDiv = document.querySelector('.tiptap');
  
      if (contentEditableDiv) {
        contentEditableDiv.innerHTML = ""; // Set the content
        contentEditableDiv.innerHTML = selectedNote.content; // Set the content
      }
  ...
  const handleSave = async () => {
    ...
    const editorContent = editableDiv.innerHTML
    const trimmedContent = editorContent.trim();
      // Check if content or title is empty
      if (!trimmedContent || trimmedContent === '<p></p>' || !noteTitle.trim()) {
        alert("Both content and title must be filled out.");
        return; 
      }
  
      setLoading(true);
      try {
        const response = await saveNotesAPI({
          body: {
